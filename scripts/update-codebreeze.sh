...
# 1. VSIX
echo "[1/3] VSIX 다운로드..."
curl -fsSL -o /tmp/codebreeze-latest.vsix "$BASE_URL/codebreeze-latest.vsix"
...
curl -fsSL -o "$HOME/codebreeze-bridge.zip" "$BASE_URL/codebreeze-bridge.zip"

# 3. 브라우저 확장 CRX (PC Chrome/Edge용)
echo "[3/3] 브라우저 확장 CRX 다운로드..."
curl -fsSL -o "$HOME/codebreeze-bridge.crx" "$BASE_URL/codebreeze-bridge.crx"
